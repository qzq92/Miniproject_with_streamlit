...
from src.transformer.dataset import TransformerDataset
...
def datapipeline(batch_size,input_variables,output_sequence_length,enc_seq_len,step_size):
    df1 = pd.read_csv('./data/train_GzS76OK/fulfilment_center_info.csv')
    df2 = pd.read_csv('./data/train_GzS76OK/meal_info.csv')
    df3 = pd.read_csv('./data/train_GzS76OK/train.csv')
    ...
    # feature engineering
    df['price_adjustment'] = df['checkout_price'] - df['base_price']
    # keep specific columns
    df = df[['week', 'base_price' , 'price_adjustment', 'num_orders']]
    # groupby week
    df = df.groupby('week').sum()

    training_data = df.loc[:116] # week range from 1 to 116
    test_data = df.loc[117:] # week range from 117 to 145

    # downcast columns 
    training_data = to_numeric_and_downcast_data(training_data)
    test_data = to_numeric_and_downcast_data(test_data)
    
    # get indices to split data 
    ...
    dec_seq_len = output_sequence_length # length of decoder input is equal to length of target seq
    target_idx = training_data[input_variables].columns.get_loc("num_orders")

    processed_training_data = TransformerDataset(
    ...
    processed_test_data = TransformerDataset(
    ...
    train_dataloader = DataLoader(processed_training_data, batch_size)
    test_dataloader = DataLoader(processed_test_data, len(test_data))


    return train_dataloader, test_dataloader

def tft_datapipeline():
    df1 = pd.read_csv('./data/train_GzS76OK/fulfilment_center_info.csv')
    df2 = pd.read_csv('./data/train_GzS76OK/meal_info.csv')
    df3 = pd.read_csv('./data/train_GzS76OK/train.csv')
    df = pd.merge(df3, df2, on='meal_id',how='left')
    ...
    ct = ColumnTransformer([
        ('somename', StandardScaler(), ['base_price', 'price_adjustment'])
    ], remainder='passthrough')

    training_data =  pd.DataFrame(ct.fit_transform(training_data), columns = ['base_price','price_adjustment','num_orders','week','group'])
    test_data = pd.DataFrame(ct.transform(test_data), columns = ['base_price','price_adjustment','num_orders','week','group'])
    training_data = training_data.astype({'week': int})
    test_data = test_data.astype({'week': int})
    return training_data, test_data, StandardScaler()
